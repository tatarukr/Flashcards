criaCartao(
    'Programação'
    'O que é Python?'
    'O Python é uma linguagem de programação'
)

criaCartao(
    'Geografia',
    'Qual a capital da Alemanha?'
    'A capital da Alemanha é Berlim'
)

criaCartao(
    'História',
    'Quem criou a CLT (consolidação das leis do trabalho)?'
    'Getúlio Vargas'
)

criaCartao(
    'Lingua inglesa'
    'Como se diz oi em Inglês?'
    'Oi em ingles é HI (RAI)'
)

criaCartao(
    'Matemática'
    'Oque é a Hipotenuza?'
    'O maior lado de um triângulo retângulo'
)

criaCartao(
    'Biologia',
    'Oque significa a sigla DNA?'
    'Ácido Desoxirribonucleico'
)

criaCartao(
    'Ed. Financeira'
    'Oque é Bitcoin?'
    'Uma criptomoeda'
)

criaCartao(
    'Filosofia'
    'Que filósofo é conhecido pela alegoria filosófica "Mito da Caverna"?'
    'Platão'
)

criaCartao(
    'Física'
    'Qual a primeira lei de Newton'
    'Lei da Inércia'
)

criaCartao(
    'Robótica (Física 1)'
    'O que é o Arduino?'
    'Arduino é uma placa que você pode programar para controlar luzes, sensores e motores'
)

criaCartao(
    'Matemática 1'
    'Oque é Marketing?'
    'Marketing é divulgar e vender produtos ou ideias'
)

criaCartao(
    'Biologia 1'
    'Qual é a segunda lei de Mendel?'
    'A segunda lei de Mendel é a Lei da Segregação Independente'
)




